...
class Connection(Evented):
    ...
    @property
    def channel_id(self):
        if hasattr(self.config, "CHANNEL_ID"):
            return self.config.CHANNEL_ID
        else:
            response = requests.get(f"{self.config.MIXER_URI}channels/{self.config.CHANNEL_NAME}?fields=id")
            if response.status_code == 200:
                self.config.CHANNEL_ID = response.json()["id"]
                return self.config.CHANNEL_ID
            else:
                raise Exception(f"Could not retrieve channel id : {response.json().get('message')}")
